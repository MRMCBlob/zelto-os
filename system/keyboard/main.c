...
static ZView keyboard_grid(KbdState *s) {
    ...
    // Row 4: symbols toggle, space (wide), enter, hide.
    ZView row4 = HStack(
        action_key(s->symbols ? "ABC" : "?123", on_symbols, 1.6f, sp),
        action_key("space", on_space, 5.0f, z_rgba(0x2b, 0x33, 0x40, 0xff)),
        action_key("enter", on_enter, 1.8f, sp),
        action_key("v", on_hide_key, 1.4f, sp),
